...
function validateTraceInput(trace: unknown): void {
  if (
    !isObject(trace) ||
    !Array.isArray(trace.cqIds) ||
    !trace.cqIds.length ||
    trace.cqIds.some((id) => !nonempty(id)) ||
    new Set(trace.cqIds).size !== trace.cqIds.length ||
    !Array.isArray(trace.elements)
  )
  ...
  for (const element of trace.elements) {
    if (
      !isObject(element) ||
      !absoluteIri(element.id) ||
      elements.has(element.id) ||
      typeof element.kind !== "string" ||
      !Object.hasOwn(YONSEI_TYPES, element.kind) ||
      !Array.isArray(element.cq_ids) ||
      !element.cq_ids.length ||
...
export function validateGeneration(value: unknown): GenerationRequest {
  if (
    !isObject(value) ||
    !["openai", "anthropic"].includes(String(value.provider)) ||
    !["neon", "tao", "yonsei"].includes(String(value.method)) ||
    typeof value.stageId !== "string" ||
    !/^(neon-(0[1-9]|1[0-9]|20)|tao-0[1-8]|yonsei-0[1-9])$/.test(value.method + "-" + value.stageId) ||
    ...
    typeof value.messages.system !== "string" ||
    typeof value.messages.user !== "string" ||
    !value.messages.user.trim() ||
    typeof value.previousOntology !== "string"
  )
    fail("INVALID_REQUEST", "제공업체·단계·프롬프트 형식을 확인하세요.")
  if (value.yonseiTrace !== undefined) {
    if (value.method !== "yonsei" || !["08", "09"].includes(value.stageId) || value.purpose === "few-shot") fail("INVALID_REQUEST", "CQ 추적 정보는 Yonsei 08·09 단계 실행에서만 사용할 수 있습니다.")
...
export function normalizeResponse(value: unknown, provider: Provider, model: string, requestId?: string): ApiCompletion {
  ...
  if (reason === "length" || reason === "max_tokens")
    fail("OUTPUT_TRUNCATED", "모델 출력이 토큰 한도에서 잘렸습니다. 범위를 줄이거나 서버의 출력 한도를 조정하세요. 불완전한 결과는 저장하지 않았습니다.", 422)
  if (!["stop", "end_turn", "stop_sequence"].includes(reason) || !text.trim())
    fail("EMPTY_OR_UNSUPPORTED_RESPONSE", "텍스트 응답이 없거나 도구 실행이 필요한 응답입니다. 이 GUI는 텍스트 프롬프트 실행만 지원합니다.", 422)
  if (typeof input !== "number" || !Number.isSafeInteger(input) || input < 0 || typeof output !== "number" || !Number.isSafeInteger(output) || output < 0)
    fail("INVALID_USAGE", "API 사용량 응답 형식이 올바르지 않습니다.", 502)
  return {
    object: "chat.completion",
    model,
    choices: [{ index: 0, message: { role: "assistant", content: text }, finish_reason: reason }],
    usage: { prompt_tokens: input, completion_tokens: output },
    execution: { provider, ...(requestId ? { requestId } : {}) }
  }
}
...
export async function handleGeneration(request: Request, env: ApiEnvironment, transport: typeof fetch = fetch, timeoutMs = 120000): Promise<Response> {
  ...
  try {
    ...
    while (true) {
      ...
      if (size > MAX_BODY) {
        await reader.cancel()
        fail("REQUEST_TOO_LARGE", "요청이 6MB를 초과했습니다.", 413)
      }
    ...
    try {
      ...
      fail("INVALID_REQUEST", "JSON 요청을 읽지 못했습니다.")
    }
    ...
    if (model.startsWith("sk-") || model === key || !/^[a-zA-Z0-9][a-zA-Z0-9_.:/-]{0,150}$/.test(model))
      fail("INVALID_MODEL_CONFIG", "모델 설정에 올바른 모델 ID를 입력하세요. 키를 모델 항목에 넣으면 안 됩니다.")
    const messages = [{ role: "user", content: input.messages.user }]
    const payload = openai
      ? { model, messages: [{ role: "system", content: input.messages.system }, ...messages], max_completion_tokens: 8192, stream: false, store: false }
    ...
    if (!upstream.ok) {
      ...
      const [code, message] =
        status === 401
          ? ["AUTHENTICATION_ERROR", "API 키가 올바르지 않거나 만료되었습니다. 선택한 제공업체의 키를 확인하고 서버를 재시작하세요."]
          : status === 403
            ? ["PERMISSION_ERROR", "API 또는 모델에 접근할 권한이 없습니다. 계정·프로젝트 권한을 확인하세요."]
            : status === 429
              ? ["RATE_OR_QUOTA_LIMIT", "요청 한도 또는 사용 가능 크레딧을 초과했습니다. 제공업체의 사용량·결제 상태를 확인하세요."]
              : status === 400 || status === 404
                ? ["MODEL_OR_REQUEST_ERROR", "모델 ID 또는 요청 옵션이 지원되지 않습니다. 모델 접근 권한과 입력 길이를 확인하세요."]
