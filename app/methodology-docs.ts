...
export const METHODOLOGY_DOCS: Record<MethodologyKey, MethodologyDoc> = {
  yonsei: {
    ...
    inputs: "도메인 문서(TXT/MD 또는 직접 입력), 페르소나, 도메인 이름, 키워드를 준비합니다. Few-shot은 필요할 때 직접 생성하거나 자동 실행 중 생성할 수 있으며, 수동 단계 실행에서는 생략할 수 있습니다.",
    flow: [
    ...
    studio: [
      "01–08에서 Few-shot 생성과 현재 단계 실행은 별도입니다. 수동 실행에서는 예시를 비워 둘 수 있으며, 두 작업은 API 모드에서 각각 과금 가능한 별도 호출입니다.",
      "자동 실행은 각 단계의 유효한 Few-shot이 있으면 재사용하고, 없으면 먼저 생성한 뒤 단계를 실행합니다. 시작 시 예시 생성까지 포함한 외부 전송·과금을 확인합니다.",
      "Few-shot이나 이전 출력이 없어도 단계를 자유롭게 살펴볼 수 있습니다. 실제 단계 실행에는 필요한 이전 산출물·CQ·온톨로지가 있어야 합니다.",
