...
export function MethodologyDialog({ initialMethod, onClose }: { initialMethod: MethodologyKey; onClose: () => void }) {
  ...
  return <dialog ref={dialogRef} className="methodology-dialog" aria-labelledby="methodology-title"
    ...
    <header className="methodology-header">
      <div><p>방법론 가이드</p><h2 id="methodology-title">방법론 알아보기</h2></div>
    ...
    <div ref={bodyRef} className="methodology-body" id="methodology-content">
      <article key={selected} aria-labelledby="methodology-name">
        ...
        {doc.paper ? <p className="methodology-source">근거: <a href={doc.paper.url} target="_blank" rel="noopener noreferrer">{doc.paper.title} ↗</a><br />{doc.paper.edition} · {doc.paper.sections}</p>
          : <p className="methodology-source">사용자 제안 · Ontology Studio 초기 구현</p>}
        ...
        <section><h4>{doc.paper ? "논문에서 확인한 점과 한계" : "구현 범위와 한계"}</h4><p>{doc.evaluation}</p><p>{doc.limitations}</p>
          {doc.paper && <a className="methodology-reference" href={doc.paper.url} target="_blank" rel="noopener noreferrer">원문에서 방법론·실험 결과 확인 ↗</a>}
        </section>
