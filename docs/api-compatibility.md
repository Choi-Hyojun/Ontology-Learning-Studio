...
Yonsei는 01–08의 Few-shot 생성(`purpose: "few-shot"`)과 단계 실행을 분리합니다. Few-shot과 01–07의 개념 JSON은 Turtle 조각을 포함하더라도 온톨로지 스냅샷으로 저장하지 않습니다. 08·09는 전체 Turtle을 검증하고 이전 스냅샷을 교체합니다. GUI가 함께 보내는 `yonseiTrace`를 기준으로 기존 클래스·프로퍼티 IRI/종류, `relatedCQ` 주석과 알려진 CQ ID를 검증하며 누락·변경 시 `INVALID_CQ_TRACE`로 거부합니다. 03–07의 CQ 근거/요소 JSON 구조 검증은 브라우저에서 완료 처리 전에 수행합니다.

화면에서 저장하는 v4 로그는 세 방법론의 모드·제공업체·실제 응답·요청 시도 수·오류 이력 및 Yonsei Few-shot 프롬프트·결과를 보존합니다. v1/v2/v3 로그도 계속 복원할 수 있습니다. 복원은 항상 일시정지 상태입니다. 모드/제공업체 변경은 확인 후 현재 결과와 생성된 Few-shot을 초기화하되 과거 이력은 보존합니다.
